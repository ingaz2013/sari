...
export async function getStaffMemberById(id: number): Promise<StaffMember | null> {
  const db = await getDb();
  if (!db) return null;
  const results = await db.select().from(staffMembers).where(eq(staffMembers.id, id)).limit(1);
  return results[0] || null;
}
...
export async function getStaffMembersByMerchant(merchantId: number): Promise<StaffMember[]> {
  const db = await getDb();
  if (!db) return [];
...
export async function getActiveStaffByMerchant(merchantId: number): Promise<StaffMember[]> {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(staffMembers)
    .where(and(
...
export async function deleteStaffMember(id: number): Promise<void> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.update(staffMembers).set({ isActive: 0 }).where(eq(staffMembers.id, id));
}
